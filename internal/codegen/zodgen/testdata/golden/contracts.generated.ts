...
// ── Shared ──
export const PropField = z.object({ default: z.string(), description: z.string(), name: z.string(), options: z.array(z.string()), required: z.boolean().default(false), type: z.string() });

// ── Component Contract ──
export const ComponentContract = z.object({ name: z.string(), category: ComponentCategory });
export type ComponentContract = z.infer<typeof ComponentContract>;

// ── Section Contract ──
export const SectionContract = z.object({ key: z.string(), family: SectionFamily, variant: z.string(), title: z.string(), props: z.array(PropField) });
...
export const PageDefinition = z.object({ slug: z.string(), title: z.string(), page_type: PageType, sections: z.array(PageDefinitionSectionRef), seo: PageDefinitionSeo, legal: PageDefinitionLegalMeta.nullable().optional(), collection: z.string().nullable().optional() }).superRefine((v, ctx) => {
...
export const AppShell = z.object({ name: z.string(), kind: AppShellKind, router: z.enum(["react-router", "tanstack-router", "none"]), providers: z.array(z.string()), auth: AppShellAuth.nullable().optional(), site_config: z.string().nullable().optional() }).superRefine((v, ctx) => {
...
export const CollectionField = z.object({ name: z.string(), of_schema: z.string().nullable().optional(), of_type: UiFieldType.nullable().optional(), options: z.array(z.string()).nullable().optional(), required: z.boolean().default(false), type: UiFieldType }).superRefine((v, ctx) => {
...
export const Collection = z.object({ slug: z.string(), kind: UiCollectionType, labels: CollectionLabels, routing: CollectionRouting, gallery_section: z.string(), detail_sections: z.array(z.string()), fields: z.array(CollectionField) });
