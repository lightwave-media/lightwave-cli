...
func infraStatusHandler(_ context.Context, args []string, _ map[string]any) error {
	if len(args) < 1 {
		return errors.New("usage: lw infra status <domain-id>")
	}

	return errors.New("infra status: not yet wired (use `lw aws ecs status` for ECS health, or `lw deploy status <env>`)")
}
