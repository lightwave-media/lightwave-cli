...
func Analyze(sessionID string) error {
	...
	// Step 6b: Load time-anchored docker logs (if generated by LIGA-282)
	syncedLogs := loadSyncedLogs(sessionID)

	// Step 7: Send to Claude
	fmt.Printf("  Analyzing with Claude (%d frames)...\n", len(framePaths))
	result, err := AnalyzeWithClaude(context.Background(), sessionID, transcript, framePaths, serverLogs, syncedLogs)
...
// loadSyncedLogs reads docker.synced.jsonl and returns it as raw JSONL text for the Claude prompt.
// Returns empty string if the file doesn't exist (sessions recorded before LIGA-282 shipped).
func loadSyncedLogs(sessionID string) string {
	data, err := os.ReadFile(DockerSyncedPath(sessionID))
	if err != nil {
		return ""
	}
